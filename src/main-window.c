...
static void main_window_timeline_cb(GtkRadioMenuItem *item, MainWindow *main_window);
static void main_window_set_default_timeline(MainWindow *main_window, gchar *timeline);
static void main_window_retrieve_default_timeline(void);
...
static MainWindowPriv *main_window_priv=NULL;
...
static void main_window_class_init(MainWindowClass *klass){
	...
	g_type_class_add_private(object_class, sizeof(MainWindowPriv));
}
...
static void main_window_init(MainWindow *singleton_main_window){
	...
	main_window_priv=GET_PRIV(main_window);
	main_window_priv->widgets_connected=NULL;
	main_window_priv->widgets_disconnected=NULL;
	main_window_priv->tweet_list_glist=NULL;
	unset_selected_tweet();
	main_window_priv->tabs_to_right_align=g_strdup("\t");
}
...
static void main_window_finalize(GObject *object){
	...
	program_timeout_remove(&main_window_priv->size_timeout_id, _("main window configuration"));
	program_timeout_remove(&main_window_priv->timeout_id_status_bar, _("status bar message"));
	
	g_list_free(main_window_priv->widgets_connected);
	g_list_free(main_window_priv->widgets_disconnected);
	main_window_tweet_lists_destroy();
	if(main_window_priv->tabs_to_right_align) uber_free(main_window_priv->tabs_to_right_align);
...
static void main_window_setup(void){
	...
	ui=gtkbuilder_get_file(
				GtkBuilderUI,
					"main_window", &main_window_priv->window,
					
					"main_menubar", &main_window_priv->menubar,
					"file", &main_window_priv->menu_file,
					"edit", &main_window_priv->menu_edit,
					
					"view", &main_window_priv->menu_view,
					"view_public_timeline", &main_window_priv->timeline_public,
					"view_friends_timeline", &main_window_priv->timeline_friends,
					"view_my_timeline", &main_window_priv->timeline_mine,
					"view_direct_messages", &main_window_priv->timeline_dm,
					"view_mentions", &main_window_priv->timeline_mentions,
					"view_replies", &main_window_priv->timeline_replies,
					"view_favorites_timeline", &main_window_priv->timeline_favorites,
					
					"online_service_request", &main_window_priv->menu_online_service_request,
					"online_service_request_menu_friends_manager", &main_window_priv->online_service_request_menu_friends_manager,
					"online_service_request_menu_timelines", &main_window_priv->online_service_request_menu_timelines,
					"online_service_request_menu_follow", &main_window_priv->online_service_request_menu_follow,
					"online_service_request_menu_unfollow", &main_window_priv->online_service_request_menu_unfollow,
					"online_service_request_menu_block", &main_window_priv->online_service_request_menu_block,
					"online_service_request_menu_tweets", &main_window_priv->online_service_request_menu_tweets,
					"online_service_request_menu_profile", &main_window_priv->online_service_request_menu_profile,
					
					"help", &main_window_priv->menu_help,
					/* end menu items. */
					
					/* start:main toolbar */
	
					"main_toolbar", &main_window_priv->main_toolbar,
					
					"search_combo_box_entry", &main_window_priv->search_combo_box_entry,
					"search_list_store", &main_window_priv->search_list_store,
					"search_button", &main_window_priv->search_button,
					...
					"tweet_list_notebook", &main_window_priv->tweet_list_notebook,
					
					"tweet_vpaned", &main_window_priv->tweet_vpaned,
					"expand_box", &main_window_priv->expand_box,
					"tweet_view_vbox", &main_window_priv->tweet_view_vbox,
					
					"main_statusbar", &main_window_priv->statusbar,
				NULL
	);
	
	//main_window_priv->search_tree_model=gtk_combo_box_get_model(GTK_COMBO_BOX(main_window_priv->search_combo_box_entry));
	//main_window_priv->online_services_tree_model=gtk_tree_view_get_model(main_window_priv->online_services_tree_view);
	
	g_signal_connect_after(main_window_priv->window, "key-press-event", G_CALLBACK(tweets_hotkey), NULL);
	...
	gtkbuilder_connect( ui, main_window,
					...
					"view_public_timeline", "activate", main_window_timeline_cb,
					"view_friends_timeline", "activate", main_window_timeline_cb,
					"view_my_timeline", "activate", main_window_timeline_cb,
					"view_direct_messages", "activate", main_window_timeline_cb,
					"view_mentions", "activate", main_window_timeline_cb,
					"view_replies", "activate", main_window_timeline_cb,
					"view_favorites_timeline", "activate", main_window_timeline_cb,
	...
	if(!gconfig_if_bool(PREFS_TWEET_VIEW_DIALOG, FALSE))
		main_window_priv->tweet_view=tweet_view_new(NULL);
	else
		main_window_priv->tweet_view=tweet_view_new(main_window_priv->window);
	
	/*tweet_view stuff.*/
	main_window_priv->tweet_view_embed=tweet_view_get_embed();
	main_window_priv->tweet_view_window=tweet_view_get_window();
	
	if(!gconfig_if_bool(PREFS_TWEET_VIEW_DIALOG, FALSE)){
		gtk_widget_reparent(GTK_WIDGET(main_window_priv->tweet_view_embed), GTK_WIDGET(main_window_priv->tweet_view_vbox));
		gtk_widget_show(GTK_WIDGET(main_window_priv->tweet_view_embed));
	}else{
		gtk_widget_hide(GTK_WIDGET(main_window_priv->expand_box));
	}
	...
	if(!gconfig_if_bool(PREFS_UI_HIDDEN, FALSE))
		gtk_widget_show(GTK_WIDGET(main_window_priv->window));
	else
...
static TweetList *main_window_tweet_lists_new(const gchar *timeline){
	...
	main_window_priv->tweet_list_glist=g_list_last(main_window_priv->tweet_list_glist);
	tweet_list=main_window_priv->tweet_list_glist->data;
	main_window_priv->tweet_list_glist=g_list_first(main_window_priv->tweet_list_glist);
	
	gint page=gtk_notebook_append_page_menu(main_window_priv->tweet_list_notebook, GTK_WIDGET(tweet_list_get_child(tweet_list)), GTK_WIDGET(tweet_list_get_tab(tweet_list)), GTK_WIDGET(tweet_list_get_menu(tweet_list)) );
	tweet_list_set_page(tweet_list, page);
	
	return tweet_list;
}/*main_window_tabs_and_tweet_lists_new("/replies.xml");*/

TweetList *main_window_tweet_lists_get_timeline(const gchar *timeline){
	GList *t=NULL;
	TweetList *tweet_list=NULL;
	for(t=main_window_priv->tweet_list_glist; t; t=t->next)
		if(g_str_equal(tweet_list_get_timeline((TweetList *)t->data), timeline)){
			tweet_list=(TweetList *)t->data;
			gtk_notebook_set_current_page(main_window_priv->tweet_list_notebook, tweet_list_get_page(tweet_list));
			return tweet_list;
		}
...
TweetList *main_window_tweet_lists_get_page(gint page, gboolean close){
	...
	return tweet_list;
}/*main_window_tweet_lists_get_page(0, TRUE|FALSE);*/
...
void main_window_tweet_lists_close(void){
	GList *tl=NULL;
	for(tl=main_window_priv->tweet_list_glist; tl; tl=tl->next)
		main_window_tweet_lists_close_page(tweet_list_get_page((TweetList *)tl->data));
}/*main_window_tweet_lists_stop();*/

void main_window_tweet_lists_close_page(gint page){
	TweetList *tweet_list=main_window_tweet_lists_get_page(page, TRUE);
	gtk_notebook_remove_page(main_window_priv->tweet_list_notebook, page);
	main_window_priv->tweet_list_glist=g_list_remove(main_window_priv->tweet_list_glist, tweet_list);
...
static void main_window_tweet_lists_destroy(void){
	...
	for(tl=main_window_priv->tweet_list_glist; tl; tl=tl->next){
		...
		gtk_notebook_remove_page(main_window_priv->tweet_list_notebook, tweet_list_get_page(tweet_list));
		g_object_unref(tweet_list);
	}
	g_list_free(main_window_priv->tweet_list_glist);
}/*main_window_tweet_lists_destroy();*/
...
void main_window_tweet_view_set_embed(GtkToggleButton *toggle_button, gpointer user_data){
	...
	if(use_tweet_view_dialog){
		if(gtk_widget_get_parent(GTK_WIDGET(main_window_priv->tweet_view_embed))==GTK_WIDGET(main_window_priv->tweet_view))
			return;
		
		debug("Displaying TweetView as a stand alone dialog & setting TweetView's parent window..");
		gtk_widget_reparent(GTK_WIDGET(main_window_priv->tweet_view_embed), GTK_WIDGET(main_window_priv->tweet_view));
		window_present(GTK_WINDOW(main_window_priv->tweet_view_window), TRUE);
		g_object_add_weak_pointer(G_OBJECT(main_window_priv->tweet_view_window), (gpointer)&main_window_priv->tweet_view_window);
		gtk_window_set_transient_for(GTK_WINDOW(main_window_priv->tweet_view_window), main_window_priv->window);
		gtk_widget_hide(GTK_WIDGET(main_window_priv->expand_box));
	}else{
		if(gtk_widget_get_parent(GTK_WIDGET(main_window_priv->tweet_view_embed))==GTK_WIDGET(main_window_priv->tweet_view_vbox))
			return;
		
		debug("Embeding TweetView's into %s main window.", PACKAGE_NAME);
		gtk_widget_reparent(GTK_WIDGET(main_window_priv->tweet_view_embed), GTK_WIDGET(main_window_priv->tweet_view_vbox));
		gtk_widget_show(GTK_WIDGET(main_window_priv->expand_box));
		gtk_widget_show(GTK_WIDGET(main_window_priv->tweet_view_vbox));
		gtk_widget_show(GTK_WIDGET(main_window_priv->tweet_view_embed));
		gtk_widget_hide(GTK_WIDGET(main_window_priv->tweet_view_window));
	}
...
GtkWindow *main_window_get_window(void){
	return main_window_priv->window;
}/*main_window_get_window*/

GtkPaned *main_window_get_tweet_paned(void){
	return GTK_PANED(main_window_priv->tweet_vpaned);
...
GtkMenuItem *main_window_get_menu(const gchar *menu){
	if((g_str_equal(menu, "file")) ) return GET_PRIV(main_window)->menu_file;
	if((g_str_equal(menu, "edit")) ) return GET_PRIV(main_window)->menu_edit;
	if((g_str_equal(menu, "view")) ) return GET_PRIV(main_window)->menu_view;
	if((g_str_equal(menu, "online_service_request")) ) return GET_PRIV(main_window)->menu_online_service_request;
	if((g_str_equal(menu, "help")) ) return GET_PRIV(main_window)->menu_help;
	return NULL;
...
static void main_window_destroy_cb(GtkWidget *window, MainWindow *main_window){
	...
	gtk_widget_destroy( GTK_WIDGET(GET_PRIV(main_window)->window) );
}

static gboolean main_window_delete_event_cb(GtkWidget *window, GdkEvent *event, MainWindow *main_window){
	if(gtk_status_icon_is_embedded(main_window_priv->status_icon)) {
...
static void main_window_toggle_visibility(void){
	...
	visible=window_get_is_visible(GTK_WINDOW(main_window_priv->window));
	
	if(visible && gtk_status_icon_is_embedded(main_window_priv->status_icon)) {
		geometry_save();
...
static void main_window_timeline_cb(GtkRadioMenuItem *item, MainWindow *main_window){
	...
	if(GTK_CHECK_MENU_ITEM(main_window_priv->timeline_public)->active){
		main_window_tweet_lists_get_timeline(API_TIMELINE_PUBLIC);
		return;
	}
	
	if(GTK_CHECK_MENU_ITEM(main_window_priv->timeline_mine)->active){
		main_window_tweet_lists_get_timeline(API_TIMELINE_MINE);
		return;
	}
	
	if(GTK_CHECK_MENU_ITEM(main_window_priv->timeline_favorites)->active){
		main_window_tweet_lists_get_timeline(API_FAVORITES);
		return;
	}
	
	if(GTK_CHECK_MENU_ITEM(main_window_priv->timeline_dm)->active){
		main_window_tweet_lists_get_timeline(API_DIRECT_MESSAGES);
		return;
	}
	
	if(GTK_CHECK_MENU_ITEM(main_window_priv->timeline_mentions)->active){
		main_window_tweet_lists_get_timeline(API_MENTIONS);
		return;
	}
	
	if(GTK_CHECK_MENU_ITEM(main_window_priv->timeline_replies)->active){
		main_window_tweet_lists_get_timeline(API_REPLIES);
		return;
	}
	
	if(GTK_CHECK_MENU_ITEM(main_window_priv->timeline_friends)->active){
		main_window_tweet_lists_get_timeline(API_TIMELINE_FRIENDS);
		return;
	}
...
static void main_window_services_cb(GtkWidget *widget, MainWindow *main_window){
	online_services_dialog_show(GTK_WINDOW(main_window_priv->window));
}
...
static void main_window_preferences_cb(GtkWidget *widget, MainWindow *main_window){
	preferences_dialog_show(GTK_WINDOW(main_window_priv->window));
}

static void main_window_about_cb(GtkWidget *widget, MainWindow *main_window){
	about_dialog_new(GTK_WINDOW(main_window_priv->window));
}

static void main_window_help_contents_cb(GtkWidget *widget, MainWindow *main_window){
	help_show(GTK_WINDOW(main_window_priv->window));
}
...
static void main_window_status_icon_popup_menu_cb(GtkStatusIcon *status_icon, guint button, guint activate_time, MainWindow *main_window){
	gboolean       show=window_get_is_visible(GTK_WINDOW(main_window_priv->window));
	
	g_signal_handlers_block_by_func(main_window_priv->popup_menu_show_main_window,
									 main_window_show_hide_cb, main_window);
									
	gtk_toggle_action_set_active(main_window_priv->popup_menu_show_main_window, show);
	
	g_signal_handlers_unblock_by_func(main_window_priv->popup_menu_show_main_window,
									   main_window_show_hide_cb, main_window);
									
	gtk_menu_popup(GTK_MENU(main_window_priv->popup_menu),
					NULL, NULL,
					gtk_status_icon_position_menu,
					main_window_priv->status_icon,
					button,
					activate_time);
}
...
static void main_window_status_icon_create_menu(void){
	...
	main_window_priv->popup_menu_show_main_window=gtk_toggle_action_new("tray_show_app", _("_Show "), NULL, NULL);
	g_signal_connect(G_OBJECT(main_window_priv->popup_menu_show_main_window), "toggled", G_CALLBACK(main_window_show_hide_cb), main_window);
	...
	gtk_menu_shell_append(GTK_MENU_SHELL(main_window_priv->popup_menu), w);
...
static void main_window_status_icon_create(void){
	main_window_priv->status_icon=gtk_status_icon_new_from_icon_name("get2gnow");
	g_signal_connect(main_window_priv->status_icon,
					  "activate",
	...
	gtk_status_icon_set_visible(main_window_priv->status_icon, TRUE);
}
...
static gboolean configure_event_timeout_cb(GtkWidget *widget){
	...
	main_window_priv->size_timeout_id=0;
...
static gboolean main_window_window_configure_event_cb(GtkWidget *widget, GdkEventConfigure *event, MainWindow *main_window){
	...
	main_window_priv->size_timeout_id=g_timeout_add(500, (GSourceFunc)configure_event_timeout_cb, widget);
...
static void main_window_login(void){
	...
	if(!online_services_login(online_services)){
		online_services_dialog_show(main_window_priv->window);
		return;
	}
	
	main_window_retrieve_default_timeline();
}/*main_window_login*/
...
static void main_window_set_default_timeline(MainWindow *main_window, gchar *timeline){
	...
	if(G_STR_EMPTY(timeline)) {
		...
		return gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(main_window_priv->timeline_friends), TRUE);
	}
	
	debug("Selecting default timeline's radio button: %s", timeline);
	
	if((g_str_equal(timeline, API_TIMELINE_FRIENDS)) ){
		gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(main_window_priv->timeline_friends), TRUE);
		return;
	}

	if((g_str_equal(timeline, API_MENTIONS)) ){
		gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(main_window_priv->timeline_mentions), TRUE);
		return;
	}

	if((g_str_equal(timeline, API_DIRECT_MESSAGES)) ){
		gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(main_window_priv->timeline_dm), TRUE);
		return;
	}

	if((g_str_equal(timeline, API_FAVORITES)) ){
		gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(main_window_priv->timeline_favorites), TRUE);
		return;
	}

	if((g_str_equal(timeline, API_TIMELINE_MINE)) ){
		gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(main_window_priv->timeline_mine), TRUE);
		return;
	}
	
	if((g_str_equal(timeline, API_TIMELINE_PUBLIC)) ){
		gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM( main_window_priv->timeline_public), TRUE);
		return;
	}
...
static void main_window_retrieve_default_timeline(void){
	...
	if(first_run){
		gtk_notebook_remove_page(main_window_priv->tweet_list_notebook, 0);
		first_run=FALSE;
	}
	...
	if(open_home_page){
		...
		main_window_tweet_lists_get_timeline(timeline);
	}
	main_window_set_default_timeline(main_window, timeline);
	uber_free(timeline);
}
...
static void main_window_selected_tweet_widgets_setup(GtkBuilder *ui){
	...
	main_window_priv->selected_tweet_image_menu_items=list;
}/*main_window_selected_tweet_widgets_setup(ui);*/
...
static void main_window_connection_items_setup(GtkBuilder *ui){
	...
	main_window_priv->widgets_connected=list;
	...
	main_window_priv->widgets_disconnected=list;
}/*main_window_connection_items_setup(ui);*/

void main_window_state_on_connection(gboolean connected){
	if(!( main_window_priv && main_window_priv->widgets_connected && main_window_priv->widgets_disconnected ))
		return;
	...
	for(l=main_window_priv->widgets_connected; l; l=l->next)
		gtk_widget_set_sensitive( GTK_WIDGET(l->data), connected );
	
	for(l=main_window_priv->widgets_disconnected; l; l=l->next)
		gtk_widget_set_sensitive( GTK_WIDGET(l->data), !connected );
...
void main_window_selected_tweet_image_menu_items_show(gboolean selected_tweet){
	if(!(main_window_priv && main_window_priv->selected_tweet_image_menu_items )) return;
	
	GList *selected_tweet_image_menu_items;
	for(selected_tweet_image_menu_items=main_window_priv->selected_tweet_image_menu_items; selected_tweet_image_menu_items; selected_tweet_image_menu_items=selected_tweet_image_menu_items->next)
...
gboolean main_window_notify_on_timeout(gpointer data){
	...
	if(!gtk_status_icon_is_embedded(main_window_priv->status_icon))
		...
		notify_notification=notify_notification_new_with_status_icon(PACKAGE_TARNAME, notification, PACKAGE_TARNAME, main_window_priv->status_icon);
...
const gchar *main_window_tabs_to_right_align(void){
	...
	gtk_window_get_size(main_window_priv->window, &w, &h);
	if(!( (( w>0 && h>0 )) && ( (test_tab_count=w/100) >1) )) return main_window_priv->tabs_to_right_align;
	if(tab_count==test_tab_count && G_STR_N_EMPTY(main_window_priv->tabs_to_right_align) ) return main_window_priv->tabs_to_right_align;
	...
	if(tabs_to_right_align_swap) uber_free(tabs_to_right_align_swap);
	main_window_priv->tabs_to_right_align=tabs_to_right_align;
	
	return main_window_priv->tabs_to_right_align;
