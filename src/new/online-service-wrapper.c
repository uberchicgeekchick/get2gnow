...
OnlineServiceWrapper *online_service_wrapper_new(OnlineService *service, SoupMessage *xml, RequestMethod request_method, const gchar *requested_uri, guint8 attempt, OnlineServiceSoupSessionCallbackReturnProcessorFunc online_service_soup_session_callback_return_processor_func, OnlineServiceSoupSessionCallbackFunc callback, gpointer user_data, gpointer form_data){
	...
	online_service_wrapper->xml_doc=NULL;
	online_service_wrapper->xml_root_node=NULL;
	online_service_wrapper->request_failed=FALSE;
...
static void online_service_wrapper_run(OnlineServiceWrapper *online_service_wrapper, SoupSession *session, SoupMessage *xml){
	if(!( online_service_wrapper->service && online_service_wrapper->callback && online_service_wrapper->can_run && online_service_wrapper->xml_doc)) return;
...
}/*online_service_wrapper_run(online_service_wrapper, session, xml, &doc);*/
...
void *online_service_wrapper_callback(SoupSession *session, SoupMessage *xml, OnlineServiceWrapper *online_service_wrapper){
	...
	if(!( (online_service_wrapper->request_failed=www_xml_error_check(service, online_service_wrapper->requested_uri, xml, &online_service_wrapper->xml_doc, &online_service_wrapper->xml_root_node, &error_message)) && doc ))
		status=_("[Failed]");
	else
		status=_("[Success]");
...
static void online_service_wrapper_soup_session_callback_return_processor_func_default(OnlineServiceWrapper *online_service_wrapper, SoupMessage *xml, xmlDoc **doc, gpointer soup_session_callback_return_gpointer){
...
const xmlDoc *online_service_wrapper_get_xml_doc(OnlineServiceWrapper *online_service_wrapper){
	if(!(online_service_wrapper && online_service_wrapper->service && online_service_wrapper->xml_doc )) return NULL;
	return online_service_wrapper->xml_doc;
}/*online_service_wrapper_get_xml_doc(online_service_wrapper);*/

const xmlNode *online_service_wrapper_get_xml_root_node(OnlineServiceWrapper *online_service_wrapper){
	if(!(online_service_wrapper && online_service_wrapper->service && online_service_wrapper->xml_root_node )) return NULL;
	return online_service_wrapper->xml_root_node;
}/*online_service_wrapper_get_xml_root_node(online_service_wrapper);*/

gboolean online_service_wrapper_did_request_failed(OnlineServiceWrapper *online_service_wrapper){
	if(!(online_service_wrapper && online_service_wrapper->service && online_service_wrapper->request_failed )) return FALSE;
	return TRUE;
}/*online_service_wrapper_did_request_failed(online_service_wrapper);*/
...
void online_service_wrapper_free(OnlineServiceWrapper *online_service_wrapper, gboolean free_queue){
	...
	if(online_service_wrapper->xml_doc)
		xmlFreeDoc(doc);
	xmlCleanupParser();
