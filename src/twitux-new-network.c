...
/* Start: (c) 2009 Kaity G. B. <uberChick@uberChicGeekChick.Com> */
static gboolean twitux_network_get_more_users(SoupSession *session, SoupMessage *msg, GList **all_users);
static GList *twitux_network_get_pages_as_glist(gchar *api_page_url);
...
/* End: (c) 2009 Kaity G. B. <uberChick@uberChicGeekChick.Com> */
...
twitux_network_get_user(const gchar *username)
{
	...
	if( (!( (username) && (user_id=g_strdup(username)) && twitux_conf_get_string( (twitux_conf_get()), TWITUX_PREFS_AUTH_USER_ID, &user_id ) )) || G_STR_EMPTY(user_id) ) {
		if(user_id) g_free(user_id);
		return;
	}

	user_timeline=g_strdup_printf(TWITUX_API_TIMELINE_USER, user_id);
...
GList *twitux_network_get_friends(void){
	...
	if(! (user_friends=twitux_network_get_pages_as_glist(TWITUX_API_FOLLOWING)) )
		return NULL;
	
	twitux_network_make_users_list((gboolean)TRUE);
	return user_friends;
}
...
GList *twitux_network_get_followers(void){
	...
	if(! (user_followers=twitux_network_get_pages_as_glist(TWITUX_API_FOLLOWERS)) )
		return NULL;
...
static GList *twitux_network_get_pages_as_glist(gchar *api_page_url){
	...
	while(fetching){
		page++;
		msg=soup_message_new( "GET",(g_strdup_printf("%s?page=%d", api_page_url, page)) );
		soup_session_send_message(soup_connection, msg);
		fetching=twitux_network_get_more_users(soup_connection, msg, &all_users);
	}
	...
	return g_list_sort(all_users,(GCompareFunc) twitux_network_sort_users);
}
...
static void twitux_network_make_users_list(gboolean friends){
	...
	if(!friends)
	/* Friends retrived */
	return (user_friends ? twitux_lists_dialog_load_lists(user_friends) : twitux_app_set_statusbar_msg(_("Users parser error.")) );
}
/* End: (c) 2009 Kaity G. B. <uberChick@uberChicGeekChick.Com> */
						...
						gpointer     user_data)
{
	...
	twitux_debug(DEBUG_DOMAIN, "Timeline response: %i",msg->status_code);
